...
model call, no write. Lives in `app.site` rather than `tools/` so the
workspace can call the same function a corpus-wide report does
(`tools/content_census.py`) — reused, not reimplemented, which is the
whole reason this file exists in the first place: two copies of "how many
services show" would drift the way this project's own history keeps
finding.

Every number here is read off the actual pipeline (`material_from_brief`,
`plan_for`, `build_from_spec`, and the section builders' own caps).
"""
...
from app.site.render import FEATURE_CAP, _fills_rows, build_from_spec, material_from_brief, plan_for
from app.store import leads, sites

# Block kinds a section builder actually reads. Anything else published as a
# block is dropped in full — checked against the real corpus, not assumed; a
# kind absent from this set the FIRST time this ran would have been a
# silent, permanent loss with nothing here to say so.
...
def measure(conn, slug: str, lead_id: int) -> FixtureCensus:
    ...
    # Photographs: the pool is the business's own site photos first, then
    # Google's place photos (Material.images) — plan_for already tracks
    # exactly which never got spent by any section.
    pool = len(m.images)
    if pool:
        ...
        if own_photos and place:
            # Whether the business's OWN photography ever got a look-in.
            ...
            if own_used == 0:
                c.add("own_site_photos (subset of photos, informational)",
                     own_photos, 0,
                     "dropped: none of this business's own site "
                     "photography was ever selected")
